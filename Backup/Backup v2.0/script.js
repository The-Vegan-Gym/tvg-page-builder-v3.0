...
function addPortionVariationRow(variation = {}, container = null) {
    ...
    row.innerHTML = `
        ...
        <div class="portion-variation-link-row">
            <span class="portion-variation-link-icon" aria-hidden="true">
...
async function handleExportJpg() {
    ...
    try {
        ...
        for (let pageIndex = 0; pageIndex < recipePages.length; pageIndex += 1) {
            ...
            const macroBarOverlapHeight = heroSection && macroBar
                ? Math.max(0, heroSectionHeight - macroBar.offsetTop)
                : 0;
            const heroRect = heroSection ? {
                x: 0,
                y: 0,
                width: heroSection.offsetWidth * scaleX,
                height: Math.max(0, (heroSectionHeight - macroBarOverlapHeight) * scaleY)
            } : null;
...
function parseMasterPaste(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (currentSection === 'META') {
            ...
            // parts[1] is meal type - skip for now
